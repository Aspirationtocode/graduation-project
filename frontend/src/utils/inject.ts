...
import { bindArgs } from "src/utils/bindArgs";
 ...
 *
 * @param target - If is a string, then it's a namespace to which injector will be bound to, if an object then
...
export function inject(
  namespace: string
): <T extends Object>(target: T) => void;
  ...
  public get<T>(injectionConstructor: WithConstructor<T>): any {
    if (!this.map.has(injectionConstructor)) {
      this.map.set(
        ...
        createInjectionWithDI(injectionConstructor, this)
      );
    }
...
export function initializeInject(
  injectionMap: InjectionMap,
  namespace: string = defaultInjectNamespace
) {
...
function realInject(
  ...
  throw new Error(
    `Inject decorator must be applied to a class, a property, ` +
...
function classDecorator(namespace: string, target: any): void {
  if (!isObject(target) || !target.constructor) {
    throw new Error(
      `@inject decorator should be applied to a class only. ${target}`
    );
  }
...
function propertyDecorator(target: any, propertyName: string) {
  if (!isObject(target) || !target.constructor) {
    throw new Error(
      `Injection must be applied to a class property only ${target}`
    );
  }
  const targetConstructor = target.constructor;
  const injectionConstructor = Reflect.getMetadata(
    "design:type",
    target,
    propertyName
  );
  ...
  Object.defineProperty(target, propertyName, {
    get() {
      const injectionNamespace =
        target.injectionNamespace || defaultInjectNamespace;
      if (!(injections[injectionNamespace] instanceof InjectionMap)) {
        throw new Error(
          `Please, initialize injections with initializeInject()` +
 ...
 * @return {T}
 */
export function getInjected<T>(
  namespaceOrObject: string | Object | null | undefined,
  injectionConstructor: WithConstructor<T>
): T {
  ...
  if (isObject(namespaceOrObject)) {
    ...
    injectionNamespace = namespaceOrObject
      ? String(namespaceOrObject)
      : defaultInjectNamespace;
  }
...
function parameterDecorator(target: any, parameterIndex: number) {
  ...
  const injectedParameters =
    Reflect.getMetadata(injectedParameterMetadataName, target) || [];
 ...
 */
export function createInjectionWithDI(
  injectionConstructor: WithConstructor<any>,
  injectionMap: InjectionMap
) {
  ...
  injectedCtorParameters.sort().forEach(parameter => {
    ...
    if (!injectionMap.get(constructorParameter)) {
      if (visitedInjections.has(injectionConstructor)) {
        const chains = Array.from(visitedInjections.values())
          .map(dependency => dependency.name)
          .join(" -> ");
        throw new Error(`Cyclic dependencies are found: "${chains}"`);
      }
      resolveDependencies(
...
function checkInjectionType(
  target: any,
  injectedType: any,
  propName: string | number
): injectedType is React.Component {
  const targetName =
    target.name || (target.constructor && target.constructor.name);

  if (!(injectedType && "constructor" in injectedType)) {
    throw new Error(
      `Injected type should be a class, but \'${injectedType}\' on ${targetName} ${propName}. ` +
        "Check if you have a circular injection."
    );
  }
  if (/\{\s*\[native code\]\s*\}/.test(`${injectedType}`)) {
    throw new Error(
      `Cannot inject native objects ${injectedType} on ${targetName} ${propName}`
    );
  }
